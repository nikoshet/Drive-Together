...
require('common.php');
	...
	$email=explode('=',$_SERVER['QUERY_STRING']);
	//echo "<script> alert('".$email[1]."') </script>";
?> 
<!doctype html>
<html lang="en">
  <head>
    <title>DRIVE TOGETHER - Profile</title>
    <!-- Required meta tags -->
    <meta charset="utf-8">
...
<!--!!!!!!!!!!!!!!! UserProfile  !!!!!!!!!!!!!!!!!-->	
						<?php
							...
							///////////////////
									//echo $email[1];
									$query = "SELECT fname,lname,gender,age,imagepath FROM user WHERE email='".$email[1]."' ";
												...
												<div class='col-md-8 col-xs-12'>
													...
													<div class='row'>
														...
														"<div class='col-xs-12 col-md-3 pull-right'>
															<button type='button' class='btn btn-danger btn-block' id='banUser-".$email[1]."' onclick=\"banUser('".$email[1]."')\">Ban User</button>
														</div>";} echo "
													</div>
												</div>
											";
										...
										//echo $emai[1];
										 echo "<SCRIPT type='text/javascript'> //not showing me this
										  window.location.href='index.php';
									        alert('Sorry but this email address doesnt exist in our system ');
				...
				<div class="col-md-4 col-xs-12">
					...
					<div class="row">
						<div class="col-xs-12 col-md-7">
							<div class="Profile-rate">
								...
								<b>5/5</b><span class="text-muted"> - 4 ratings</span>
					
								</p>  -->
								<em>Your rating</em></br>
					 <fieldset id='demo1' class="rating">
                        ...
                        <?php 
                        	...
                        	$vote="SELECT rate FROM rating WHERE driver_email='".$email[1]."' AND user_email='".$_SESSION["user"]["email"]."';";
                        	$v=mysqli_query($link,$vote);

            							if (mysqli_num_rows($v) > 0) {
    
										    while($votee = mysqli_fetch_assoc($v)) {
										    	$yourvote=$votee[rate];
										       if ($yourvote==1) {
										       	  	 echo "<script>  $( document ).ready(function() { alert('sdsdsd'); $('#star1').prop('checked', true); });</script> ";
										       }elseif ($yourvote==2) {
										       		echo "<script> $( window ).on( 'load', function() { $('#star2').prop('checked', true); });</script>";
										       }elseif ($yourvote==3) {
										       		echo "<script> $( window ).on( 'load', function() { $('#star3').prop('checked', true);  });</script>";
										       }elseif ($yourvote==4) {
										       		echo "<script> $( window ).on( 'load', function() { $('#star4').prop('checked', true);  });</script>";
										       }
										       elseif ($yourvote==5) {
										       		echo "<script> $( window ).on( 'load', function() { $('#star5').prop('checked', true);  });</script>";
										       }
										    }
										} 
                   ...
                   <?php echo "<input id='drivermail' type='hidden' value='".$email[1]."' name='drivermail'></input>"; ?>
							</div>
						</div>
				<div class="col-xs-12 col-md-5 pull-right">
						<em id="average">Average ratings </br>
							<?php 
								/*$servername="localhost";
						        $username="root";
						        $password="root";
						        $dbname="drive_together";
						        $link = mysqli_connect($servername, $username, $password, $dbname) or die ("Could not connect");*/
								$sqll = "SELECT AVG(rate) as average from rating where driver_email='".$email[1]."'	;	";
								$resultt = mysqli_query($link, $sqll);
								
            							if (mysqli_num_rows($resultt) > 0) {
    
										    while($roow = mysqli_fetch_assoc($resultt)) {
										    	$average=round($roow[average],2);
										        echo $average."	/ 5";
										    }
										} 
  		...
  		<script>
            $(document).ready(function () {
                    $("#demo1 .stars").click(function () {
                        var mail=$("#drivermail").val();
                        //alert(mail);
                        $.post('rating.php',{rate:$(this).val(),mail},function(average){
                           	alert('Thanks For Rating');
                           	alert(average);
                            $("#average").html("<em> Average rating </br>" +average+"</em>" );
                        });
                                $(this).attr("checked");
...
<?php
				///////////////////
				$query = "SELECT departure,arrival,datee,available_seats,value_per_seat,info FROM  trips_proposed  WHERE driver_email='".$email[1]."' ";
				$result = mysqli_query($link, $query) or die('Query failed: ' . mysqli_error($link));
				if (mysqli_num_rows($result) > 0) {
					while($row = mysqli_fetch_assoc($result)) {				
						echo "
...
<?php
				///////////////////
				$query = "SELECT passengers.trip_id,driver_email,departure,arrival,datee,value_per_seat FROM passengers JOIN trips_proposed on passengers.trip_id=trips_proposed.trip_id WHERE passenger_email='".$email[1]."' ";
				$result = mysqli_query($link, $query) or die('Query failed: ' . mysqli_error($link));
				if (mysqli_num_rows($result) > 0) {							  
					while($row = mysqli_fetch_assoc($result)) {
						echo " 
